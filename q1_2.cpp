...
int main(int argc, char* argv[]) {
    ...
    k_start = (total_perms/float(size))*rank;
    k_end = (total_perms/float(size))*(rank+1);
    ...
    for (long long k = k_start; k < k_end; ++k) {
        vector<unsigned char> perm = get_kth_permutation_fenwick(k, n, fact);
        ...
        for (int t = 1; t <= n - 1; ++t) {
            vector<unsigned char> parent = parent1(perm, t,identity);
            for (unsigned char val : parent)
                buffer << (int)val;
            buffer << "  ";
        }
