...
// after you’re done
    ...
    for (auto &pair : permToRecord) {
        ...
        for (int t = 1; t < n; ++t) {
            vector<unsigned char> perm = parent1(key, t, identity);  // Store permutation directly
            auto it = permToRecord.find(perm);
            uint32_t parent_idx = (it==permToRecord.end() ? /*error*/0 : it->second.idx);
            rec.parents[t-1] = parent_idx;
            // buffer << parent_idx;
            // buffer << "  ";
        }
...
for (uint32_t k = start; k < end; ++k) {
    auto &perm = permutations[k - start];
    // lookup its record
    auto it = permToRecord.find(perm);
    if (it == permToRecord.end()) {
        cerr << "Error: no record for k=" << k << "\n";
        continue;
    }
    auto &rec = it->second;

    // print k and the permutation
    cout << "k=" << k << "  perm=[";
    for (auto c : perm) cout << int(c) << ",";
    cout << "]  parents=[";

    // print all parent‑indices
    for (int t = 0; t < n - 1; ++t) {
        // auto a = get_kth_permutation(rec.parents[t] , n);
        // for (auto c : a) cout << int(c);
        // cout << " ";
        cout << rec.parents[t] << " ";
    }
    cout << "]\n";
